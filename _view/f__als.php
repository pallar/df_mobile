...
<!--	<h3 class="text-center">Lots</h3>-->
	<div class="alert navbar-brand">
		<button class="btn btn-primary" style="<?php echo $_btn_style;?>" ng-show="show_form" ng-click="L_add_form_show();">Додати&nbsp;&nbsp;<span class="glyphicon glyphicon-plus" aria-hidden="true"></span></button>
	</div>
...
<!-- Form template which is used to insert data -->
	<form class="alert alert-warning" id="l_add_form" name="l_add" <?php echo $_form_content_style;?> hidden>
...
<!-- Form template which is used to edit and update data -->
	<form class="alert alert-warning" id="l_edit_form" name="l_edit" <?php echo $_form_content_style;?> hidden>
		<div <?php echo $_card_content_style;?>>
<?php
include( "f__al_e.htm" );
?>
...
<!-- Lots List -->
	<div class="table-responsive" id="ls_list" <?php echo $_content_style;?>>
		...
		<tr ng-repeat="detail in details | filter:search_query">
			<td style="padding-left:16px;"><button class="btn btn-warning" style="border-radius:0; padding-left:10px; padding-right:10px;" title="Edit" ng-click="L_edit_form_show(detail);"><span class="glyphicon glyphicon-edit"></span></button></td>
			<td><button class="btn btn-danger" style="border-radius:0; padding-left:10px; padding-right:10px;" title="Delete" ng-disabled="detail.id==1 || detail.locked.length>0" ng-click="db_L_delete(detail);"><span class="glyphicon glyphicon-trash"></span></button></td>
			<td>{{detail.nick}}</td>
			<td>{{detail.num}}</td>
		</tr>
		</table>
