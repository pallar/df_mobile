...
<!--	<h3 class="text-center">Subgroups</h3>-->
	<div class="alert navbar-brand">
		<button class="btn btn-primary" style="border-radius:0;" ng-show="show_form" ng-click="Sg_add_form_show();">Add&nbsp;&nbsp;<span class="glyphicon glyphicon-plus" aria-hidden="true"></span></button>
	</div>
...
<!-- Form template which is used to insert data -->
	<form class="alert alert-warning" id="sg_add_form" name="sg_add" style="margin:0; padding:0;" hidden>
...
<!-- Form template which is used to edit and update data -->
	<form class="alert alert-warning" id="sg_edit_form" name="sg_edit" style="margin:0; padding:0;" hidden>
		<div <?php echo $_card_content_style;?>>
<?php
...
<!-- Subgroups List -->
	<div class="table-responsive" id="sgs_list" <?php echo $_content_style;?>>
		...
		<tr ng-repeat="detail in details | filter:search_query">
			<td style="padding-left:16px;"><button class="btn btn-warning" style="border-radius:0;" title="Edit Subgroup" ng-click="Sg_edit_form_show(detail);"><span class="glyphicon glyphicon-edit"></span></button></td>
			<td><button class="btn btn-danger" style="border-radius:0;" title="Delete Subgroup" ng-disabled="detail.id==1" ng-click="db_Sg_delete(detail);"><span class="glyphicon glyphicon-trash"></span></button></td>
			<td>{{detail.nick}}</td>
			<td>{{detail.num}}</td>
		</tr>
		</table>
