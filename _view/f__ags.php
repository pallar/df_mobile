<?php
$ANGULAR_IS_USED=1;
$HTML_COMMENT="<!-- DF_ajs: Groups Form -->";
$HTML_TAG="<html ng-app='f_gs'>";
...
<nav1>
	<div id='cssmenu1'>
		<ul>
			...
			<li class='active'><a href='f__ags.php'><span>Секції</span></a></li>
...
<!--	<h3 class="text-center">Groups</h3>-->
	<div class="alert navbar-brand">
		<button class="btn btn-primary" style="border-radius:0; padding-left:10px; padding-right:10px;" ng-show="show_form" ng-click="G_add_form_show();">Додати&nbsp;&nbsp;<span class="glyphicon glyphicon-plus" aria-hidden="true"></span></button>
	</div>
...
<!-- Form template which is used to insert data -->
	<form class="alert alert-warning" id="g_add_form" name="g_add" <?php echo $_form_content_style;?> hidden>
...
<!-- Form template which is used to edit and update data -->
	<form class="alert alert-warning" id="g_edit_form" name="g_edit" <?php echo $_form_content_style;?> hidden>
		<div <?php echo $_card_content_style;?>>
<?php
include( "f__ag_e.htm" );
?>
...
<!-- Groups List -->
	<div class="table-responsive" id="gs_list" <?php echo $_content_style;?>>
		...
		<tr ng-repeat="detail in details | filter:search_query">
			<td style="padding-left:16px;"><button class="btn btn-warning" style="border-radius:0; padding-left:10px; padding-right:10px;" title="Edit" ng-click="G_edit_form_show(detail);"><span class="glyphicon glyphicon-edit"></span></button></td>
			<td><button class="btn btn-danger" style="border-radius:0; padding-left:10px; padding-right:10px;" title="Delete" ng-disabled="detail.id==1 || detail.locked.length>0" ng-click="db_G_delete(detail);"><span class="glyphicon glyphicon-trash"></span></button></td>
			<td>{{detail.nick}}</td>
			<td>{{detail.num}}</td>
		</tr>
		</table>
