<?php
/*
JsHttpRequest: backend for JavaScript DHTML loader
version 5.x
...
class JsHttpRequest {
	...
	var $_contentTypes=array(
		...
		"xml"=>"text/plain",//In XMLHttpRequest mode we must return text/plain for Opera 8.0.
		"form"=>"text/html",
		""=>"text/plain",
	);
	...
	*/
	function JsHttpRequest($enc) {
		//QUERY_STRING is in form of: PHPSESSID=<sid>&a=aaa&b=bbb&JsHttpRequest=<id>-<loader>, where <id> is a request ID, <loader> is a loader name, <sid> is a session ID (if present), PHPSESSID is session parameter name (by default="PHPSESSID").
		//Parse QUERY_STRING.
		...
		//If true then generate a debug message and exit.
		$file=$line=null;
		if (headers_sent($file, $line)) {
			trigger_error(
				...
				."Please note that JsHttpRequest uses its own Content-Type header and fails "
				."if this header cannot be set. See header() function documentation for more details",
				E_USER_ERROR
			);
			exit();
		}
	...
	/*
	string getJsCode()
	Returns JavaScript part of the library.
	*/
	function getJsCode() {
		return file_get_contents(dirname(__FILE__).'/jshttprq.js');
	}
	...
	Examples:
	"windows-1251" is set plain encoding (non-windows characters (hieroglyphs, etc. are ignored).
	"windows-1251 entities" is set windows encoding, BUT additionally replace: "&"->"&amp;", hieroglyph->&#XXXX; entity.
	*/
	function setEncoding($enc) {
		...
		//Manually parse QUERY_STRING because of Unicode's %uXXXX.
	...
	If entities are used (see setEncoding()), no '&' character is quoted, only '"', '>' and '<' (we presume that '&' is already quoted by an input reader function).
	...
	*/
	function php2js($a=false) {
		...
		if (is_scalar($a)) {
			...
			//PHP's "1" and 1 are equal for all PHP operators, but JS's "1" and 1 are not. So if we pass "1" or 1 from the PHP backend, we should get the same result in the JS frontend (string).
	...
	*/
	function _correctSuperglobals() {
		//In case of FORM loader everything is already parsed by PHP.
		...
		//Library frontend sets "application/octet-stream" for that purpose, see JavaScript code.
		$source=array(
	...
	*/
	function _obHandler($text) {
		...
		//Content-type header. In XMLHttpRequest mode we must return text/plain for Opera 8.0.
		...
		if ($this->LOADER!="xml") {
			...
			//If top.JsHttpRequestGlobal is not defined, loading is aborted and iframe is removed, so do not call dataReady().
	...
	If a key contains non-ASCII characters, this function sets $this->_toUtfFailed=true, because array_walk_recursive() cannot modify array keys.
	*/
	...
	/*
	Decode all %uXXXX entities in string or array.
	...
	*/
	...
	/*
	If there is no ICONV, try to decode 1-byte characters manually (for most popular charsets only).
	*/
