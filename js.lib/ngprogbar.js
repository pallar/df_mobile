...
angular.module('ngProgress.provider', ['ngProgress.directive'])
 .service('ngProgress', function() {
	...
	return ['$document', '$window', '$compile', '$rootScope', '$timeout', function($document, $window, $compile, $rootScope, $timeout) {
		...
		// Start the animation and add between 0 - 5 percent to loading
		...
		// Set the height of the progressbar (any valid CSS value). E.g. '10px', '1em' or '1%'.
		...
		// Set the color of the progressbar and it's shadow (any valid HTML color).
		...
		// Return on how many percent the progressbar is at.
		this.status = function() {
			return this.count;
		};

		// Stop the progressbar at it's current location.
		this.stop = function() {
			clearInterval(this.intervalCounterId);
		};

		// Set the progressbar percentage (a number between 0 - 100).
		...
		// Reset the progressbar to percetage 0 and therefore will be hided after it's rollbacked.
		this.reset = function() {
			clearInterval(this.intervalCounterId);
			this.count = 0;
		...
		// Jump to 100% progress and fade away the progressbar.
		...
		// Get the current element the progressbar is attached to.
